/**
 * Logos de Google e GitHub para os botões de login social.
 *
 * `Icone` (base.tsx) usa a fonte Material Symbols, que não tem marca de
 * terceiro nenhuma - por isso aqui é SVG inline, como qualquer botão "Entrar
 * com X" pede (o desenho oficial de cada marca, não um ícone genérico).
 */

export function MarcaGoogle({ className = "h-[18px] w-[18px]" }: { className?: string }) {
  return (
    <svg viewBox="0 0 18 18" className={className} aria-hidden>
      ...
      <path
        fill="#EA4335"
        d="M9 3.58c1.32 0 2.51.45 3.44 1.35l2.58-2.58A8.54 8.54 0 0 0 9 0 9 9 0 0 0 .96 4.96l3 2.33C4.67 5.16 6.66 3.58 9 3.58Z"
      />
    </svg>
  );
}

export function MarcaGithub({ className = "h-[18px] w-[18px]" }: { className?: string }) {
  return (
